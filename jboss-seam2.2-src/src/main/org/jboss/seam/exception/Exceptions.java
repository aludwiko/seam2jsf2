...
public class Exceptions
{
      ...
      if (stream != null)
      {
         ...
         for (final Element exception : elements)
         {
            ...
            catch (IllegalArgumentException ex)
            {
               ...
               sb.append(" is configured with an invalid log-level.  Acceptable " + "values are: fatal,error,warn,info,debug,trace. " + "A default level of 'error' has been configured instead.");
            ...
            {
               if(className==null)
                  handler = createHandler(exception, null);
               else
   ...
   private ExceptionHandler createHandler(Element exception, final Class clazz)
   {
      ...
      if (redirect != null)
      {
         ...
         Severity severity = severityName == null ? FacesMessage.SEVERITY_INFO : Pages.getFacesMessageValuesMap().get(severityName.toUpperCase());
         return new ConfigRedirectHandler(viewId == null ? null : Expressions.instance().createValueExpression(viewId, String.class), clazz, endConversation, message, severity);
      }
      ...
      Element handleClass = exception.element("handle-class");
      if (handleClass != null)
      {
         try
         {
            String className = handleClass.attributeValue("class-name");
            Class<?> handleClazz = Reflections.classForName(className);
            Object ins = handleClazz.newInstance();
            return (ExceptionHandler) ins;
         }
         catch (Exception e)
         {
         }
   ...
   /**
    * @return the exception handler list, which supports addition and removal of
    *         handlers
    */
